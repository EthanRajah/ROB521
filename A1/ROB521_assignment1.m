...
for i = 1:nS
    samples = [];
    % Sample a point uniformly in the maze
    x = rand() * col + 0.5;
    y = rand() * row + 0.5;
    samples = [x, y];
    % Sample a point from a Gaussian distribution centered at the uniform sample
    x_gauss = x + 0.1 * randn();
    y_gauss = y + 0.1 * randn();
    samples = [samples; x_gauss, y_gauss];
    % If one of the two points is in collision, keep the one not in collision
    min_distances = MinDist2Edges(samples, map);
    if min_distances(1) > 0.1 && min_distances(2) > 0.1
        valid_samples = [valid_samples; samples(1,:)]; % Keep only q
    elseif min_distances(1) > 0.1
        valid_samples = [valid_samples; samples(1,:)]; % Keep q if it's free
    elseif min_distances(2) > 0.1
        valid_samples = [valid_samples; samples(2,:)]; % Keep q' if it's free
    end
end
...
while ~isempty(pq)
    ...
    for i = 1:length(neighbors)
        ...
        if CheckCollision([node_x, node_y], [neighbor_x, neighbor_y], map)
            % In collision so remove edge and continue
            edges(neighbors(i),:) = [];
            continue;
        end
        % Get neighbor index in milestones
