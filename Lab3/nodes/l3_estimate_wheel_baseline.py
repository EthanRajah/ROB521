...
class wheelBaselineEstimator():
    ...
    def startStopCallback(self, msg):
        if self.isMoving is False and np.absolute(msg.angular.z) > 0:
            ...
            # Lec 20, Slide 20
            numerator = (2 * np.pi) * (self.del_right_encoder - self.del_left_encoder) / TICKS_PER_ROTATION
            denominator = 2 * np.pi * NUM_ROTATIONS
            separation = (numerator / denominator) * (WHEEL_RADIUS / 2)
