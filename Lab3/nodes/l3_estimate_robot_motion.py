...
class WheelOdom:
    ...
    def sensor_state_cb(self, sensor_state_msg):
        ...
        if self.last_enc_l is None:
            ...
            left_wheel_speed = 2 * np.pi * WHEEL_RADIUS * (le - self.last_enc_l) / ENC_TICKS
            right_wheel_speed = 2 * np.pi * WHEEL_RADIUS * (re - self.last_enc_r) / ENC_TICKS
            theta_bar = euler_from_ros_quat(self.pose.orientation)[2]

            self.pose.position.x = self.pose.position.x + np.cos(theta_bar) * (WHEEL_RADIUS / 2) * right_wheel_speed + np.cos(theta_bar) * (WHEEL_RADIUS / 2) * left_wheel_speed
            self.pose.position.y = self.pose.position.y + np.sin(theta_bar) * (WHEEL_RADIUS / 2) * right_wheel_speed + np.sin(theta_bar) * (WHEEL_RADIUS / 2) * left_wheel_speed
            delta_theta = (WHEEL_RADIUS / 2 * BASELINE) * right_wheel_speed - (WHEEL_RADIUS / 2 * BASELINE) * left_wheel_speed
            self.pose.orientation = ros_quat_from_euler([0, 0, theta_bar + delta_theta])
            
            mu_dot = np.array([self.pose.position.x, self.pose.position.y, delta_theta])
