...
for i=2:size(t_laser,1)
    ...
    for n=1:nparticles
        ...
        for beam=1:2         
            ...
            % Previous steps computed the prediction step of the particle filter. Need to implement corrections step based on the two laser beams
            laser_range = y_laser(i,j);
            if isnan(laser_range) || laser_range > y_laser_max
                continue;
            end

            % Get set of occupied cells from ray
            range_pixel = laser_range / ogres;
            theta_laser = theta_particle(n) + angles(j);
            theta_laser = atan2(sin(theta_laser), cos(theta_laser));
            x_particle_pixel = round((x_particle(n) - ogxmin) / ogres);
            y_particle_pixel = round((y_particle(n) - ogymin) / ogres);
            x_end = round(x_particle_pixel + range_pixel * cos(theta_laser));
            y_end = round(y_particle_pixel + range_pixel * sin(theta_laser));
            x_idxs = [];
            y_idxs = [];
            x_step = x;
            y_step = y;
            ...
            % Find closest occupied cell to current particle position
            threshold = 0.6;
            ...
            laser_pred = sqrt((x_end - x_particle(n))^2 + (y_end - y_particle(n))^2);

            % Create Gaussian distribution for the laser range based on prediction and actual measurement. Allows us to reweigh particles based on
            % how well they match the actual laser range
            w_particle(n) = w_particle(n) * w_gain * normpdf(laser_range, laser_pred, sqrt(laser_var));
